...
|---------------------|---------------------|
| Uwierzytelnianie oparte na certyfikacie | Masz certyfikat X509 skonfigurowany dla jednostki lub użytkownika usługi Azure Active Directory (AAD). Aby dowiedzieć się więcej, zobacz [Get started with certificate-based authentication in Azure Active Directory (Wprowadzenie do uwierzytelniania opartego na certyfikacie w usłudze Azure Active Directory)]. |
...
Aby uzyskać więcej informacji na temat korzystania z zestawu Azure SDK dla języka Go w usłudze Azure Stack, zobacz [Use API version profiles with Go in Azure Stack](/azure/azure-stack/user/azure-stack-version-profiles-go) (Korzystanie z profilów wersji interfejsu API za pomocą języka Go w usłudze Azure Stack).
